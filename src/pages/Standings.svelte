        ...
        if (_teams)
            teams = _teams.filter(t => {
                return t.name !== "RFA" && t.name !== "FA" && t.name !== "WW" && t.name !== "N/A" &&
                    Object.keys(t.standings.s11).includes(league);
            }).sort((t1, t2) => {
                if (t1.standings.s11[league].win > t2.standings.s11[league].win) return -1;
                else if (t1.standings.s11[league].win < t2.standings.s11[league].win) return 1;
                else return 0;
            })
    }
</script>
...
<Collection once={true} path="teams" on:data={(e)=>_teams = e.detail.data}>
    ...
    {#each teams as team, i (team.name)}
        <div animate:flip={{duration:500}} in:fly={{x:-100}} out:fly={{x:100}} class="uk-width-1-1">
            <TeamRow {team} season={seasonNum} {league}/>
        </div>
    {/each}
</Collection>
</Body>
