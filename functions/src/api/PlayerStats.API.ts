...
export async function getStats(){
    ...
    const [playerStatSheet] = await sheets.sheetValues(
        playerSheetId,
        [
            `Player Stats!A2:${sheets.columnToLetter(playerStatMeta.columnCount)}${playerStatMeta.rowCount ?? 2 + 1}` // Start at A2 because A1 just says "MLE"
        ]
    );
    const playerStatHeaders: string[][] = playerStatSheet.values?.splice(0, playerStatMeta.frozenRowCount - 1).map(ha => ha.map(h => sheets.clean(h))) ?? [];
